...
# Apply the item modifier
    ...
    $execute if block ~-1 ~ ~-1 #detonate:mineables/$(tool) run execute as @p[scores={.detonate.others.users=1},distance=..8] run function detonate:mine/item_modifiers/unbreaking
